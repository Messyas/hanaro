...
| **4. Scrap e settings** | Implementação concluída: settings usa a API pública de tipos de defeito; revisão em lote, fila, workflow de anexos, catálogo de modelos e listagem têm responsabilidades separadas; a feature está organizada em `list/`, `review/` e `templates/`. Rotas e API pública foram preservadas. A cobertura de regressão continua pendente conforme o [plano de execução de scrap-base](plano-execucao-scrap-base.md). | Builds, checker e fronteira pública passaram; executar os testes relevantes antes do aceite final. |
