...
| --- | --- | --- | --- |
| FE-SOLID-01 | P1 | In progress — export configuration, API client, editor, list, capability, preview API/state/transitions and workspace state extracted; legacy history markup removed; source selection/versioning and export-history restoration/polling/state delegated | Split Reports vertically. The route shell no longer owns export polling, DOM download, source mutation, and every editor section; current URLs and behavior remain unchanged. |
