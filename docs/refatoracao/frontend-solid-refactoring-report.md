...
- [ ] Extract `ReportExportCoordinator` for polling and `BrowserDownloadPort` for download/object URL behavior.
...
| --- | --- | --- | --- |
| FE-SOLID-01 | P1 | Not started | Split Reports vertically. The route shell no longer owns export polling, DOM download, source mutation, and every editor section; current URLs and behavior remain unchanged. |
...
- Expand the route-scoped `ReportEditorStore` into the owner of editor state and effects, without turning it into another god object.
