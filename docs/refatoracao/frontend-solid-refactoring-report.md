...
| --- | --- | --- | --- |
| FE-SOLID-01 | P1 | In progress — export configuration, API client, editor, list, capability, preview API/state/transitions including derived analytics and error handling, and period-close workspace transformations/coordination extracted; legacy history markup removed; source selection/versioning and export-history restoration/polling/state delegated; source, publication, catalog, and period-close HTTP capabilities extracted from the broad ReportsService; Action Plans now consumes the catalog and publication capabilities it needs | Split Reports vertically. The route shell no longer owns export polling, DOM download, source mutation, and every editor section; current URLs and behavior remain unchanged. |
...
- [x] Extract Period Close candidate and mutation coordination from the route shell.
