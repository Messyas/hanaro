...
from src.app.models.rate_limit.schemas import (
    RateLimitRead,
    RateLimitUpdate,
)
from src.app.support.common.exceptions import ResourceExistsError, ResourceNotFoundError
from src.app.support.common.utils.error_handler import handle_exception
from src.app.support.rate_limit.dependencies import RateLimitServiceDep
from src.infrastructure.auth.http_exceptions import (
    DuplicateValueException,
    HTTPException,
    NotFoundException,
)
from src.infrastructure.dependencies import AsyncSessionDep, CurrentSuperUserDep

router = APIRouter(tags=["Rate Limits"])

UNEXPECTED_ERROR_DETAIL = "An unexpected error occurred"
RATE_LIMIT_NOT_FOUND_DETAIL = "Rate limit not found"
...
@router.get(
    ...
    responses={
        ...
        404: {"description": RATE_LIMIT_NOT_FOUND_DETAIL},
    },
...
async def get_rate_limit(
    ...
    try:
        ...
        raise NotFoundException(RATE_LIMIT_NOT_FOUND_DETAIL)
    except Exception as e:
        http_exception = handle_exception(e)
        if http_exception:
            raise http_exception
