...
env_path = next((path for path in env_paths if os.path.isfile(path)), None)
if env_path:
    logger.info(f"Using environment file at: {env_path}")
else:
    logger.info("No environment file mounted; using process environment variables")

# Passing None is the supported environment-only mode and avoids Starlette's
# misleading "Config file not found" warning inside Docker containers.
...
class CORSSettings(BaseSettings):
    ...
    CORS_ENABLED: bool = config("CORS_ENABLED", default=False, cast=bool)
    ...
    CORS_ALLOW_METHODS: str = config("CORS_ALLOW_METHODS", default="GET,POST,PUT,PATCH,DELETE,OPTIONS")
    CORS_ALLOW_HEADERS: str = config(
        "CORS_ALLOW_HEADERS",
...
class ProfileImageSettings(BaseSettings):
    """Validated local storage settings for user profile images."""
...
class SecuritySettings(BaseSettings):
    ...
    TRUSTED_HOSTS: str = config("TRUSTED_HOSTS", default="localhost,127.0.0.1,testserver,backend")

    @property
    def TRUSTED_HOSTS_LIST(self) -> list[str]:
        """Return the explicit Host header allowlist."""
