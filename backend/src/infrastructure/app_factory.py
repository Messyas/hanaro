...
def _validate_cors_configuration(
    ...
    *,
    ...
    if allow_credentials and any(
        "*" in values for values in (origins, methods, headers)
    ):
        raise ValueError(
...
def lifespan_factory(
    ...
    async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
        ...
        try:
            ...
            if (
                isinstance(settings, RateLimiterSettings)
                and settings.RATE_LIMITER_ENABLED
            ):
...
def _setting_option(
    explicit_value: Any, settings: Settings, setting_name: str, default: Any
) -> Any:
...
def _build_contact_metadata(settings: Settings) -> dict[str, str]:
    ...
    contact = {
        ...
        "email": _first_configured_value(
            settings, "API_CONTACT_EMAIL", "CONTACT_EMAIL"
        ),
...
def _build_application_metadata(
    ...
    *,
    ...
    metadata: dict[str, Any] = {
        "openapi_prefix": _setting_option(
        ...
        "openapi_url": _setting_option(
            openapi_url, settings, "OPENAPI_URL", OPENAPI_PATH
        ),
    }
    optional_metadata = {
        "title": title
        if title is not None
        else _first_configured_value(settings, "API_TITLE", "APP_NAME"),
        "summary": summary
        if summary is not None
        else _first_configured_value(settings, "API_SUMMARY"),
        "description": description
        if description is not None
        else _first_configured_value(settings, "API_DESCRIPTION", "APP_DESCRIPTION"),
        "version": version
        if version is not None
        else _first_configured_value(settings, "API_VERSION", "VERSION"),
        "terms_of_service": terms_of_service
        if terms_of_service is not None
        else _first_configured_value(settings, "API_TERMS_OF_SERVICE"),
        "contact": contact
        if contact is not None
        else _build_contact_metadata(settings),
        "license_info": license_info
        if license_info is not None
        else _build_license_metadata(settings),
        "openapi_tags": openapi_tags
        if openapi_tags is not None
        else _configured_openapi_tags(settings),
    }
    metadata.update(
...
def _add_standard_middleware(
    application: FastAPI, settings: Settings, *, enable_gzip: bool
) -> None:
    ...
    client_cache_enabled = (
        ...
        and getattr(settings, "CLIENT_CACHE_ENABLED", False)
    )
    ...
    if enable_gzip:
        application.add_middleware(
            GZipMiddleware, minimum_size=getattr(settings, "GZIP_MINIMUM_SIZE", 1000)
        )
    ...
    if trusted_hosts:
        application.add_middleware(
            TrustedHostMiddleware, allowed_hosts=trusted_hosts, www_redirect=False
        )
...
def _cors_settings(settings: Settings, origins: list[str]) -> dict[str, Any]:
    ...
    cors_settings: dict[str, Any] = {
        ...
        "allow_headers": (
            [header.strip() for header in headers.split(",") if header.strip()]
...
def _include_documentation_routes(
    ...
    *,
    ...
    show_docs = isinstance(settings, EnvironmentSettings) and (
        ...
        or enable_docs_in_production
    )
    if not show_docs:
        return

    dependency = _documentation_dependency(
        settings, enable_docs_in_production, docs_production_dependency
    )
    docs_router = APIRouter(
        dependencies=[Depends(dependency)] if dependency is not None else []
    )
...
def create_application(
    ...
    **kwargs: Any,
    ...
    create_tables = _setting_option(
        create_tables_on_startup, settings, "CREATE_TABLES_ON_STARTUP", True
    )
    cors_enabled = _setting_option(enable_cors, settings, "CORS_ENABLED", False)
    configured_origins = _setting_option(
    ...
    application = FastAPI(
        lifespan=lifespan
        or lifespan_factory(settings, create_tables_on_startup=create_tables),
        **app_options,
    )
    ...
    if cors_enabled:
        application.add_middleware(
            CORSMiddleware, **_cors_settings(settings, configured_origins)
        )
    return application
