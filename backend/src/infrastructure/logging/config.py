...
def _context_correlation_id() -> str | None:
    """Read the correlation ID from the current async context."""
    try:
...
def _thread_correlation_id() -> str | None:
    """Read the correlation ID attached to the current thread."""
    try:
        value = getattr(threading.current_thread(), "correlation_id", None)
...
def _request_correlation_id(request: object) -> str | None:
    """Read a correlation ID from a request state or its headers."""
    state = getattr(request, "state", None)
    state_value = getattr(state, "correlation_id", None)
    if state_value:
    ...
    if headers is None:
        return None
    header_value = headers.get("x-correlation-id") or headers.get("x-request-id")
    return str(header_value) if header_value else None


def _stack_correlation_id() -> str | None:
    """Find a request in the current call stack and read its correlation ID."""
    try:
...
class CorrelationIdFilter(logging.Filter):
    ...
    def _get_correlation_id(self) -> str | None:
        ...
        """
        for resolver in (
            _context_correlation_id,
            _thread_correlation_id,
            _stack_correlation_id,
        ):
