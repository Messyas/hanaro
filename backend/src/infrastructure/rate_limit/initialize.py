...
async def initialize_rate_limiter() -> None:
    ...
    if settings.RATE_LIMITER_BACKEND == CacheBackend.MEMCACHED.value:
        ...
        memcached_backend = MemcachedBackend(
            settings=memcached_settings, fail_open=settings.RATE_LIMITER_FAIL_OPEN
        )
        rate_limiter_provider.register_backend(
        ...
        redis_backend = RedisBackend(
            settings=redis_settings, fail_open=settings.RATE_LIMITER_FAIL_OPEN
        )
        rate_limiter_provider.register_backend(
...
async def close_rate_limiter() -> None:
    ...
    if (
        settings.RATE_LIMITER_BACKEND == CacheBackend.MEMCACHED.value
        and MEMCACHED_INSTALLED
    ):
