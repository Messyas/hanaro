...
def _usage_datetime(record: dict[str, Any]) -> datetime | None:
    """Parse a usage timestamp, ignoring malformed records."""
...
def _add_usage_record(day: dict[str, Any], record: dict[str, Any]) -> None:
    """Accumulate one usage record into a daily aggregate."""
    day["requests"] += 1
    status_code = record.get("status_code", 0)
    outcome = "successful_requests" if 200 <= status_code < 300 else "failed_requests"
    day[outcome] += 1
    day["tokens"] += record.get("tokens_used", 0) or 0
...
def calculate_daily_usage(usage_records: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for record in usage_records:
        ...
        day = daily_usage.setdefault(day_key, _empty_daily_usage(day_key))
        _add_usage_record(day, record)
