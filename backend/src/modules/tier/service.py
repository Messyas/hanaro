...
class TierService:
    ...
    async def create(self, tier: TierCreate, db: AsyncSession) -> dict[str, Any]:
        ...
        if await crud_tiers.exists(db=db, name=tier_dict["name"]):
            raise ResourceExistsError(
                f"Tier with name '{tier_dict['name']}' already exists"
            )

        tier_internal = TierCreateInternal(**tier_dict)
        created_tier = await crud_tiers.create(
            db=db, object=tier_internal, schema_to_select=TierRead
        )
        if not created_tier:
    ...
    async def get_all(
        ...
        return await crud_tiers.get_multi(
            db=db, offset=skip, limit=limit, schema_to_select=TierRead, is_deleted=False
        )

    async def get_by_id(self, tier_id: int, db: AsyncSession) -> dict[str, Any]:
        """Retrieve a tier by ID."""
        tier = await crud_tiers.get(
            db=db, id=tier_id, schema_to_select=TierRead, is_deleted=False
        )
        if not tier:
    ...
    async def get_by_name(self, name: str, db: AsyncSession) -> dict[str, Any]:
        ...
        tier = await crud_tiers.get(
            db=db, name=name, schema_to_select=TierRead, is_deleted=False
        )
        if not tier:
    ...
    async def update(
        self, name: str, tier_update: TierUpdate, db: AsyncSession
    ) -> None:
        """Update a tier by name."""
        existing_tier = await crud_tiers.get(
            db=db, name=name, schema_to_select=TierRead
        )
        ...
        if "name" in update_data and update_data["name"] != name:
            if await crud_tiers.exists(db=db, name=update_data["name"]):
                raise ResourceExistsError(
                    f"Tier with name '{update_data['name']}' already exists"
                )
    ...
    async def verify_superuser(
        self, user: dict[str, Any], action: str = "manage tiers"
    ) -> None:
