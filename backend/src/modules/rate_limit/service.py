...
class RateLimitService:
    ...
    async def create(
        self, rate_limit: RateLimitCreate, tier_id: int, db: AsyncSession
    ) -> dict[str, Any]:
        ...
        if name_exists:
            raise ResourceExistsError(
                f"Rate limit with name '{rate_limit_dict['name']}' already exists"
            )

        rate_limit_internal = RateLimitCreateInternal(
            **rate_limit_dict, tier_id=tier_id
        )
    ...
    async def get_active_and_inactive_by_name(
        self, name: str, db: AsyncSession
    ) -> dict[str, Any]:
        """Get an active or inactive rate limit by name."""
        rate_limit = await crud_rate_limits.get(
            db=db, name=name, schema_to_select=RateLimitRead
        )
        if not rate_limit:
            raise ResourceNotFoundError(f"Rate limit with name '{name}' not found")
        return rate_limit
    ...
    async def update(
        self, name: str, rate_limit_update: RateLimitUpdate, db: AsyncSession
    ) -> None:
        """Update a rate limit by name."""
        existing_rate_limit = await crud_rate_limits.get(
            db=db, name=name, schema_to_select=RateLimitRead
        )
        ...
        if "name" in update_data and update_data["name"] != name:
            ...
            if name_exists:
                raise ResourceExistsError(
                    f"Rate limit with name '{update_data['name']}' already exists"
                )

        internal_update = RateLimitUpdateInternal(
    ...
    async def verify_superuser(
        self, user: dict[str, Any], action: str = "manage rate limits"
    ) -> None:
